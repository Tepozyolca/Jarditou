...
<body>
	<?php     
		...
		{		 
			...
			if (empty($_POST["pro_prix"]) && $_POST["pro_prix"] != 0) 
			{     
				$erreurform = str_split($erreurform);
			...
			if (empty($_POST["pro_stock"]) && $_POST["pro_stock"] != 0) 
			{      
				$erreurform = str_split($erreurform);
			...
			$requete = $db->prepare("UPDATE produits SET pro_cat_id=:pro_cat_id, pro_ref=:pro_ref, pro_libelle=:pro_libelle, pro_description=:pro_description, pro_prix=:pro_prix, pro_stock=:pro_stock, pro_couleur=:pro_couleur, pro_d_modif=:pro_d_modif, pro_bloque=:pro_bloque
									WHERE pro_id =:pro_id");
